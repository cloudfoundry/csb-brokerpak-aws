...
func Read() (*redis.Options, error) {
	...
	var services struct {
		RedisServices []RedisService `jsonry:"csb-aws-redis"`
	}
	...
	case 1: // ok
	case 0:
		return nil, fmt.Errorf("unable to find `csb-aws-redis` in %q", variable)
	default:
		return nil, fmt.Errorf("more than one entry for `csb-aws-redis` in %q", variable)
	}
